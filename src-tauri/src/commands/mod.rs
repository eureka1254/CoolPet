...
#[tauri::command]
pub async fn synthesize_volcano_tts(
    _app: AppHandle,
    api_key: String,
    app_id: String,
    voice_type: String,
    ...
    let (result, audio) = ai::volcano_tts::synthesize(&api_key, &app_id, &voice_type, &text).await;

    let mut json = serde_json::to_value(&result).map_err(|e| e.to_string())?;

    if let Some(audio_bytes) = audio {
        use std::io::Write;
        // Save test.mp3 next to the executable
        let mp3_path = std::path::PathBuf::from("test.mp3");
        if let Ok(mut file) = std::fs::File::create(&mp3_path) {
            ...
            if let Some(obj) = json.as_object_mut() {
                ...
                obj.insert(
                    ...
                    serde_json::Value::String(mp3_path.to_string_lossy().to_string()),
                );
            }
        }

        // Also encode audio as base64 for frontend playback
