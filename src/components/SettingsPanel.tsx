import { CheckCircle2, Eye, EyeOff, FolderOpen, Loader2, RotateCcw, Save, TestTube, Trash2, Volume2 } from "lucide-react";
...
export function SettingsPanel({ settings, dataPaths, onSave, onResetStats, onOpenDataDir, onClose }: SettingsPanelProps) {
  ...
  const [ttsAppId, setTtsAppId] = useState("7110455648");
  const [ttsVoice, setTtsVoice] = useState("zh_female_vv_uranus_bigtts");
  const [ttsText, setTtsText] = useState("你好，我是CoolPet");
  ...
  async function handleTtsTest(): Promise<void> {
    ...
    setTtsStatus(null);
    try {
      const result = await synthesizeVolcanoTts(ttsApiKey.trim(), ttsAppId.trim(), ttsVoice.trim(), ttsText.trim());

      // Build full diagnostic report
      const lines: string[] = [];
      lines.push(`HTTP Status: ${result.http_status}`);
      lines.push(`Content-Type: ${result.content_type || "(none)"}`);
      lines.push(`Body Length: ${result.body_length} bytes`);
      ...
      if (result.response_headers) {
        ...
        lines.push(result.response_headers);
      }
      if (result.body_preview) {
        lines.push(`--- Body Preview ---`);
        lines.push(result.body_preview);
      }
      if (result.audio_saved && result.audio_path) {
        lines.push(`--- Audio Saved ---`);
        lines.push(`Path: ${result.audio_path}`);
      }

      setTtsStatus(lines.join("\n"));
      ...
      if (result.success && result.audio_base64) {
        ...
        audio.onerror = () => {
          URL.revokeObjectURL(url);
          setTtsStatus((prev) => (prev ?? "") + "\n⚠ 音频播放失败");
        };
        await audio.play();
      }
    } catch (error) {
      const errMsg = getErrorMessage(error, "TTS 合成失败");
  ...
  return (
    <PanelShell title="设置" subtitle="AI 模型与系统偏好" onClose={onClose}>
      <div className="flex h-full flex-col gap-3 overflow-y-auto pr-1">
        ...
        <div className="rounded-[8px] border border-sky-100 bg-white/72 p-3">
          <span className="text-[12px] font-semibold text-slate-700">Volcano TTS</span>
          ...
          <label className="mt-2 block text-[11px] font-semibold text-slate-500">
            ...
            >
              <option value="zh_female_vv_uranus_bigtts">uranus (推荐)</option>
              <option value="zh_female_wanwan">wanwan</option>
              <option value="zh_female_xiaoyan">xiaoyan</option>
            </select>
          </label>
          ...
          {ttsStatus ? (
            <div className="mt-2 rounded-[8px] border border-sky-100 bg-white/72 px-3 py-2 text-[11px] leading-relaxed text-slate-600">
              <pre className="whitespace-pre-wrap break-all font-sans">{ttsStatus}</pre>
            </div>
          ) : null}

          <div className="mt-3">
            <button
              className="flex h-8 w-full items-center justify-center gap-1 rounded-[8px] bg-violet-500 text-[11px] font-semibold text-white disabled:opacity-50"
              type="button"
              disabled={ttsTesting}
              onClick={() => void handleTtsTest()}
            >
              {ttsTesting ? <Loader2 size={13} className="animate-spin" /> : <Volume2 size={13} />}
              试听
            </button>
          </div>
        </div>
